...
int findSpot(int h, int w, int** map_mat, int range, int IDnum, bot_info* bots, int* profitX, int* profitY){
  ...
  srand(time(NULL));
	...
	//fprintf(stderr, "profit1: %d (%d, %d) \n", profit1, profitX1, profitY1 );
	//fprintf(stderr, "profit2: %d (%d, %d) \n", profit2, profitX2, profitY2 );
	//fprintf(stderr, "profit3: %d (%d, %d) \n", profit3, profitX3, profitY3 );
	//fprintf(stderr, "profit4: %d (%d, %d) \n", profit4, profitX4, profitY4 );
  int r = rand() % 100;
	//fprintf(stderr, "R VALOR: %d \n", r);
	
	
  if (r >= 0 && r <= 40 && profit1 > 0){
		//fprintf(stderr, "SPOT: 1º \n");
    *profitX = profitX1;
    *profitY = profitY1;
    return 0;
  }
...
int fishingAction(int* spotX, int* spotY, int** mat_map, int* weight, int* stage){
  ...
  if((mat_map[*spotY][*spotX] > 11) && (mat_map[*spotY][*spotX]  <= 19) && (*weight <=9)  ){
    ...
    *weight = *weight +1;
    ...
    //fprintf(stderr, "NÃO DEVE PESCAR POIS ESTA CHEIO\n");
    return 10;
  }
  else{
    //fprintf(stderr, "NÃO DEVE PESCAR POIS NAO TEM PEIXE\n");
    return 20;
  }
  //fprintf(stderr, "erro na função fishing\n");
